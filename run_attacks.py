...
from models import ESAN, SUN, SubstructureAwareGNN, GNN, TriggerGenerator, OODDetector, SubstructureAwareGNN_CS, NewSubstructureAwareGNN
...
if len(sys.argv) > 1:
    ...
    if arg == "all":
        model_types = ["esan", "sun", "sagn", "sagn+cs", "gnn"]
    else:
        model_types = [arg]
else:
    model_types = ["esan", "sun", "sagn", "sagn+cs", "gnn"]
    ...
    for dataset_name in ["Cora", "CiteSeer", "PubMed"]:
        ...
        if model_type == "esan":
            ...
            for epoch in range(200):
                ...
                optimizer.step()
                if epoch % 10 == 0:
                    print(f"Dataset: {dataset_name}, SAGN+CS Epoch {epoch}, Loss: {loss.item():.4f}")
            model.eval()
            with torch.no_grad():
                out = model(data.x, data.edge_index)
                predictions = out.argmax(dim=1)
            ...
            gnn_types = ['GCN', 'GraphSage', 'GAT', 'SGC', 'GIN']
